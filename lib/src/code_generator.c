...
char **C_CodeGenEmit(C_AstProgram *program) {
    ...
    for (int i = 0; i < arrlen(program->function_declarations); i++) {
        char **function_lines = C_CodeGenEmitFunctionDeclaration(
...
char **C_CodeGenEmitVariableAssignment(
...
char **C_CodeGenEmitExpression(C_AstExpression *expression,
    ...
    switch (expression->type) {
        case C_CONSTANT: {
            char **constant_lines =
                C_CodeGenEmitConstant(expression->constant);
        ...
        case C_VARIABLE: {
            char **variable_lines =
                C_CodeGenEmitVariable(expression->variable);
            ADD_TO_LINES(variable_lines);
            arrfree(variable_lines);
            break;
        }
        case C_BINARY_EXPRESSION: {
            char **binary_expression_lines = C_CodeGenEmitBinaryExpression(
...
char **C_CodeGenEmitStatement(C_AstStatement *statement,
    ...
    switch (statement->type) {
        case C_STATEMENT_BLOCK: {
            char **block_lines =
                C_CodeGenEmitBlock(statement->block, current_offset);
            ADD_TO_LINES(block_lines);
            arrfree(block_lines);
            break;
        }
        case C_STATEMENT_RETURN: {
            char **return_lines = C_CodeGenEmitReturn(
        ...
        case C_STATEMENT_FUNCTION_DECLARATION: {
            char **function_declaration_lines =
                C_CodeGenEmitFunctionDeclaration(
